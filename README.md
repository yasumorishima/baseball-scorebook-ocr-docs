...
- **その前提を2つ外して測り直した（同 2026-07-28）**: 同じ照合機構（字形の向きを課金する距離）を**打球記述の分類**と**マークの有無判定**にも流用して自前で認識させたところ、**出塁/アウトの自前判定で 84%（打席単位）**、**マーク有無は 2445 枠で 94%**。**両方を自前にした状態での最終精度は 84%**（教えてもらった場合の 93% に対して）。残る前提は「どのセルが打席か」だけ
