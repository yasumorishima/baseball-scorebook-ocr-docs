...
- 🔴 **この 93% が前提にしていること（2026-07-28 に明示・自己測定）**: パイプラインは **「どのセルが打席か」「どのコーナーに進塁マークがあるか」「打者がアウトか出塁か」を正解データから受け取った上で**、コーナーの数字を読んでいる。つまり 93% は**「マークの在処を教えてもらった上での読み取り精度」**。前提を明記せずに使わない
- **その前提を2つ外して測り直した（同 2026-07-28）**: 同じ照合機構（字形の向きを課金する距離）を**打球記述の分類**と**マークの有無判定**にも流用して自前で認識させたところ、**出塁/アウトの自前判定で 84%（打席単位）**、**マーク有無は 2445 枠で 94%**。**両方を自前にした状態での最終精度は 83%**（教えてもらった場合の 93% に対して。2026-07-28 時点では 84% だったが、その後の取込みで測定対象そのものが増えたため率としては下がっている）。残る前提は「どのセルが打席か」だけ
- **設計上の知見2つ**: ①**見落としと誤検出は等価でない**——マークの見落としはその角を永久に読めなくするが、余分な検出はソルバーが「進塁しなかった走者」として処理できる。実測でも**適合率重視の分類器（269）は分類器なし（288）より悪く**、再現率側に振ると 303 まで上がる。②**一様なノイズで見積もると実際の認識器を過小評価する**——出塁判定は 15.6% 誤るが、損害は一様に 2% 誤らせた場合と同じだった（誤りがソルバーの依存していないセルに集中するため）
